    ...
    <table border="1" cellpadding="5">
        <thead>
            <tr>
                ...
                <th style="width: 10%; text-align: center;">ID. Retur</th>
                <th style="width: 13%; text-align: center;">Tanggal</th>
                <th style="width: 43%; text-align: center;">Keterangan</th>
                <th style="width: 17%; text-align: center;">ID. Pembelian</th>
...
if ($data->num_rows() > 0) {
    foreach ($data->result() as $row) {
        ...
        $dataDetail = array();
        $query      = $this->db->query("SELECT * FROM v_returpenjualan_detil WHERE idreturpenjualan='$row->idreturpenjualan' ORDER BY namaproduk ");
        if ($query->num_rows() > 0) {
        ...
        $idpenjualan = '<span>ID. ' . $row->idpenjualan . ' <br>Tgl. ' . formatHariTanggal(date('d/m/Y', strtotime($row->tglpenjualan))) . '</span>';

        $table .= '
            <tr>
                <td style="width: 5%; text-align:center;">' . $no++ . '</td>
                <td style="width: 10%; text-align: center;">' . '<b>' . $row->idreturpenjualan . '</b></td>
                <td style="width: 13%; text-align: left;">' . formatHariTanggal($row->tglreturpenjualan) . '</td>
                <td style="width: 43%; text-align: left;"><span style="font-size: 10px;">' . implode('<br>', $dataDetail) . '</span>
                </td>
                <td style="width: 17%; text-align: left;">' . $idpenjualan . '</td>
