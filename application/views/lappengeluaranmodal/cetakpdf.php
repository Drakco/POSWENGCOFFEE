...
$table = '
    ...
    <h4 style="font-size:16px;">Laporan Pengeluaran Modal</h4>
    ...
    <table border="1" cellpadding="5">
        <thead>
            <tr style="background-color:#ccc;">
                ...
                <th style="width: 16%; text-align: center;">Total Harga Modal (Rp.) /<br>Total Harga (Rp.)</th>
...
$no = 1;
$grand_total_hargamodal = 0;
$grand_total_diskon     = 0;
$grand_total_harga      = 0;

foreach ($data->result() as $row) {
    $totalharga = 0;
    $totalharga = $row->totalharga_modal - $row->totalharga_diskon;
    $table .= '
        <tr style="background-color:#eee;">
            ...
            <td style="width: 16%; text-align: right;">Rp. '.number_format($row->totalharga_modal).'</td>
    ...
    $grand_total_hargamodal += $row->totalharga_modal;
    $grand_total_diskon     += $row->totalharga_diskon;
    $grand_total_harga      += $totalharga;

    $dataDetil = $this->LapPengeluaranModal_model->getLaporanDetil($row->id);
    if ($dataDetil->num_rows() > 0) {
        foreach ($dataDetil->result() as $rowDetil) {
            $totalharga_modal_detil = 0;
            $totalharga_modal_detil = $rowDetil->qty * $rowDetil->hargamodal;
            $table .= '
                <tr>
                    ...
                    <td style="width: 17%; text-align: right;">'.$rowDetil->qty.' x Rp. '.number_format($rowDetil->hargamodal).'</td>
                    <td style="width: 16%; text-align: left;">Rp. '.number_format($totalharga_modal_detil).'</td>
...
$table .= '
            <tr style="font-weight: bold; background-color: #ddd;">
                ...
                <th style="width: 16%; text-align: right;">Rp. '.number_format($grand_total_hargamodal).'</th>
