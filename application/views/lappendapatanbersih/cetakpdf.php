...
$table = '
    ...
    <h4 style="font-size:16px;">Laporan Pendapatan Bersih</h4>
...
$no = 1;
$grand_total_hargabersih= 0;
$grand_total_diskon     = 0;
$grand_total_harga      = 0;

foreach ($data->result() as $row) {
    $totalharga = 0;
    $totalharga = $row->totalharga_bersih - $row->totalharga_diskon;
    $table .= '
        <tr style="background-color:#eee;">
            ...
            <td style="width: 16%; text-align: right;">Rp. '.number_format($row->totalharga_bersih).'</td>
    ...
    $grand_total_hargabersih+= $row->totalharga_bersih;
    $grand_total_diskon     += $row->totalharga_diskon;
    $grand_total_harga      += $totalharga;

    $dataDetil = $this->LapPendapatanBersih_model->getLaporanDetil($row->id);
    if ($dataDetil->num_rows() > 0) {
        foreach ($dataDetil->result() as $rowDetil) {
            $totalharga_jual_detil = 0;
            $hargabersih           = $rowDetil->hargajual - $rowDetil->hargamodal;
            $totalharga_jual_detil = $rowDetil->qty * $hargabersih;
            $table .= '
                <tr>
                    ...
                    <td style="width: 17%; text-align: right;">'.$rowDetil->qty.' x Rp. '.number_format($hargabersih).'</td>
...
$table .= '
            <tr style="font-weight: bold; background-color: #ddd;">
                ...
                <th style="width: 16%; text-align: right;">Rp. '.number_format($grand_total_hargabersih).'</th>
