...
$table = '
	<h3>LAPORAN BARANG BAHAN BAKU MASUK</h3>
...
if ($data->num_rows() > 0) {
	foreach ($data->result() as $row) {
		...
		$dataDetail = array();
		$query      = $this->db->query("SELECT * FROM v_barangmasuk_detail WHERE idbarangmasuk='$row->idbarangmasuk' ORDER BY namabarang ");
		if ($query->num_rows() > 0) {
		...
		$table .= '
            <tr>
                ...
                <td style="width: 10%; text-align: center;">' . '<b>' . $row->idbarangmasuk . '</b></td>
                <td style="width: 10%; text-align: left;">' . formatHariTanggal($row->tglbarangmasuk) . '</td>
                <td style="width: 43%; text-align: left;">Ket. ' . $row->keterangan . ' <br><span style="font-size: 10px;">' . implode('<br>', $dataDetail) . '</span>
                </td>
                <td style="width: 10%; text-align: right;">' . number_format($row->totalharga) . '</td>
                <td style="width: 10%; text-align: center;">' . $row->nostruk . '</td>
                <td style="width: 12%; text-align: right;">' . number_format($row->totalharga) . '</td>
            </tr>
        ';

		$total += $row->totalharga_konfirmasi;
	}
}
