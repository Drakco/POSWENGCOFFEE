...
if ($data->num_rows() > 0) {
    foreach ($data->result() as $row) {
        ...
        $dataDetail = array();
        $query      = $this->db->query("SELECT * FROM v_pembelian_detil WHERE idpembelian='$row->idpembelian' ORDER BY namaproduk ");
        if ($query->num_rows() > 0) {
        ...
        $table .= '
            <tr>
                ...
                <td style="width: 10%; text-align: center;">' . '<b>' . $row->idpembelian . '</b></td>
                <td style="width: 10%; text-align: left;">' . formatHariTanggal($row->tglpembelian) . '</td>
                ...
                <td style="width: 10%; text-align: left;">' . $row->nostruk . '</td>
