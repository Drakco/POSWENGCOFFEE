...
class Pengguna extends CI_Controller
{
	...
	var $controller     = 'Pengguna';
	var $loadViewList   = 'pengguna/list';
	var $loadViewForm   = 'pengguna/form';

	var $formNameHead   = 'Pengguna';
	var $formNameData   = 'Data Pengguna';
	...
	public function edit($primaryKey)
	{
		...
		if ($this->Pengguna_model->getById($primaryKey)->num_rows() == 0) {
			$pesan = '
                        <script type="text/javascript">
                          Swal.fire(
                          "Gagal !",
	...
	public function hapus($primaryKey)
	{
		...
		$hapus = $this->Pengguna_model->delete($primaryKey);
		if ($hapus) {
	...
	public function simpan()
	{
		...
		if ($idpengguna == '') {
			...
			$idpengguna = $this->db->query("SELECT f_idpengguna_create() AS idpengguna")->row()->idpengguna;
			...
			$simpan = $this->Pengguna_model->insert($data);
			if ($simpan) {
	...
	public function getEditData()
	{
		...
		$result                 = $this->Pengguna_model->getById($primaryKey)->row();
	...
	function getData()
	{
		$data       = $this->Pengguna_model->get_datatables();
		$dataArr    = array();
		$no         = $_POST['start'];
		...
		foreach ($data as $row) {
			...
			$arr[] = $row->idpengguna . '<br>' . $statusaktif;
			...
			$arr[]  = '
                        <a href="' . site_url($this->controller . '/edit/' . $this->encrypt->encode($row->idpengguna)) . '" class="btn btn-xs btn-warning btn-circle" title="Edit Data">
                            <i class="fa fa-edit"></i>
                        </a>  
                     
                        <a href="' . site_url($this->controller . '/hapus/' . $this->encrypt->encode($row->idpengguna)) . '" class="btn btn-xs btn-danger btn-circle" id="hapus" title="Hapus Data">
