...
class Barangmasuk extends CI_Controller
{
    ...
    public $controller   = 'Barangmasuk';
    public $loadViewList = 'barangmasuk/list';
    public $loadViewForm = 'barangmasuk/form';

    public $formNameHead = 'Barang Masuk';
    public $formNameData = 'Data Barang Masuk';
    public $formNameAdd  = 'Form Tambah Data';
    public $formNameEdit = 'Form Edit Data';

    public $menu = 'Barangmasuk';
    ...
    public function edit($primaryKey)
    {
        ...
        if ($this->Barangmasuk_model->getById($primaryKey)->num_rows() == 0) {
            $pesan = '
                        <script type="text/javascript">
                          Swal.fire(
                          "Gagal !",
    ...
    public function hapus($primaryKey)
    {
        ...
        $hapus = $this->Barangmasuk_model->delete($primaryKey);
        if ($hapus) {
    ...
    public function simpanUpload()
    {
        ...
        $simpan = $this->db->query("UPDATE barangmasuk SET foto='$foto' WHERE idbarangmasuk='$idbarangmasuk' ");
        if ($simpan) {
    ...
    public function simpan()
    {
        ...
        if ($idbarangmasuk == '') {
            ...
            $idbarangmasuk = $this->db->query("SELECT f_idbarangmasuk_create('" . date('Y-m-d') . "') AS idbarangmasuk")->row()->idbarangmasuk;

            $data = array(
                'idbarangmasuk'    => $idbarangmasuk,
                'tglbarangmasuk'   => $tglbarangmasuk,
                'nostruk'          => $nostruk,
            ...
            $simpan = $this->Barangmasuk_model->insert($data, $dataDetail);
            if ($simpan) {
            ...
            $simpan = $this->Barangmasuk_model->update($data, $idbarangmasuk, $dataDetail);
            if ($simpan) {
                echo json_encode(array(
                    'success' => true,
                    'msg'     => 'Data Berhasil Disimpan',
    ...
    public function getEditData()
    {
        ...
        $result     = $this->Barangmasuk_model->getById($primaryKey)->row();

        $data = array(
            'idbarangmasuk'    => $result->idbarangmasuk,
            'tglbarangmasuk'   => $result->tglbarangmasuk,
    ...
    {
        $data    = $this->Barangmasuk_model->get_datatables();
        $dataArr = array();
        $no      = $_POST['start'];
        ...
        foreach ($data as $row) {
            ...
            $arr[] = '<b>' . $row->idbarangmasuk . '</b>';

            $arr[] = formatHariTanggal($row->tglbarangmasuk);
            $arr[] = $row->keterangan;
            ...
            if (!empty($row->foto)) {
                $arr[] = '
                    <a href="' . base_url('uploads/' . $row->foto) . '" class="btn btn-success btn-xs" target="_blank">
                ...
                $arr[] = '
                    ...
                    data-idbarangmasuk="' . $row->idbarangmasuk . '"
                    data-foto="' . $row->foto . '">
            ...
            $arr[] = '
                        <a href="' . site_url($this->controller . '/edit/' . $this->encrypt->encode($row->idbarangmasuk)) . '" class="btn btn-xs btn-warning btn-circle" title="Edit Data">
                            <i class="fa fa-edit"></i>
                        </a>

                        <a href="' . site_url($this->controller . '/hapus/' . $this->encrypt->encode($row->idbarangmasuk)) . '" class="btn btn-xs btn-danger btn-circle" id="hapus" title="Hapus Data">
        ...
        $output = array(
            "draw"            => $_POST['draw'],
            "recordsTotal"    => $this->Barangmasuk_model->count_all(),
            "recordsFiltered" => $this->Barangmasuk_model->count_filtered(),
            "data"            => $dataArr,
        );
    ...
    public function datatablesourcedetail()
    {
        ...
        $query = $this->db->query("SELECT * FROM v_barangmasuk_detail WHERE idbarangmasuk='$idbarangmasuk' ");
        $no    = 1;
        $i     = 0;
        if ($query->num_rows() > 0) {
