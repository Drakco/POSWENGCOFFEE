...
class Kategori extends CI_Controller {
    ...
    var $controller     = 'Kategori'; 
    var $loadViewList   = 'kategori/list';
    var $loadViewForm   = 'kategori/form';

    var $formNameHead   = 'Kategori';
    var $formNameData   = 'Data Kategori';
    ...
    public function edit($primaryKey)
    {
        ...
        if ($this->Kategori_model->getById($primaryKey)->num_rows() == 0) {
            $pesan = '
                        <script type="text/javascript">
                          Swal.fire(
                          "Gagal !",
    ...
    public function hapus($primaryKey)
    {
        ...
        $hapus = $this->Kategori_model->delete($primaryKey);
        if ($hapus) {
    ...
    public function simpan()
    {
        ...
        $urisegment = str_replace(' ', '-', strtolower($this->input->post('namakategori')));

        if ($idkategori == '') {
            
            $idkategori = $this->db->query("SELECT f_idkategori_create() AS idkategori")->row()->idkategori;
            ...
            $simpan = $this->Kategori_model->insert($data);
            if ($simpan) {
    ...
    public function getEditData()
    {
        ...
        $result                 = $this->Kategori_model->getById($primaryKey)->row();
    ...
    function getData()
    {
        $data       = $this->Kategori_model->get_datatables();
        $dataArr    = array();
        $no         = $_POST['start'];
        ...
        foreach ($data as $row) {
            ...
            $arr[] = $row->idkategori.'<br>'.$statusaktif;
            ...
            $arr[]  = '
                        <a href="'.site_url( $this->controller.'/edit/'.$this->encrypt->encode($row->idkategori) ).'" class="btn btn-xs btn-warning btn-circle" title="Edit Data">
                            <i class="fa fa-edit"></i>
                        </a>  
                     
                        <a href="'.site_url( $this->controller.'/hapus/'.$this->encrypt->encode($row->idkategori) ).'" class="btn btn-xs btn-danger btn-circle" id="hapus" title="Hapus Data">
