...
class Bank extends CI_Controller
{
	...
	var $controller     = 'Bank';
	var $loadViewList   = 'bank/list';
	var $loadViewForm   = 'bank/form';

	var $formNameHead   = 'Metode Pembayaran';
	var $formNameData   = 'Data Metode Pembayaran';
	...
	public function edit($primaryKey)
	{
		...
		if ($this->Bank_model->getById($primaryKey)->num_rows() == 0) {
			$pesan = '
                        <script type="text/javascript">
                          Swal.fire(
                          "Gagal !",
	...
	public function hapus($primaryKey)
	{
		...
		$hapus = $this->Bank_model->delete($primaryKey);
		if ($hapus) {
	...
	public function simpan()
	{
		...
		if ($idbank == '') {
			...
			$idbank = $this->db->query("SELECT f_idbank_create() AS idbank")->row()->idbank;
			...
			$simpan = $this->Bank_model->insert($data);
			if ($simpan) {
	...
	public function getEditData()
	{
		...
		$result                 = $this->Bank_model->getById($primaryKey)->row();

		$data = array(
			'idbank' => $result->idbank,
			'namabank' => $result->namabank,
			'atasnama' => $result->atasnama,
	...
	function getData()
	{
		$data       = $this->Bank_model->get_datatables();
		$dataArr    = array();
		$no         = $_POST['start'];
		...
		foreach ($data as $row) {
			...
			$arr[]  = '
                        <a href="' . site_url($this->controller . '/edit/' . $this->encrypt->encode($row->idbank)) . '" class="btn btn-xs btn-warning btn-circle" title="Edit Data">
                            <i class="fa fa-edit"></i>
                        </a>  
                     
                        <a href="' . site_url($this->controller . '/hapus/' . $this->encrypt->encode($row->idbank)) . '" class="btn btn-xs btn-danger btn-circle" id="hapus" title="Hapus Data">
