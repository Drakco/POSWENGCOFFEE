...
class Ekspedisi extends CI_Controller {
    ...
    var $controller     = 'Ekspedisi'; 
    var $loadViewList   = 'ekspedisi/list';
    var $loadViewForm   = 'ekspedisi/form';

    var $formNameHead   = 'Ekspedisi';
    var $formNameData   = 'Data Ekspedisi';
    ...
    public function edit($primaryKey)
    {
        ...
        if ($this->Ekspedisi_model->getById($primaryKey)->num_rows() == 0) {
            $pesan = '
                        <script type="text/javascript">
                          Swal.fire(
                          "Gagal !",
    ...
    public function hapus($primaryKey)
    {
        ...
        $hapus = $this->Ekspedisi_model->delete($primaryKey);
        if ($hapus) {
    ...
    public function simpan()
    {
        ...
        $namaekspedisi = $this->input->post('namaekspedisi');
        ...
        if ($idekspedisi == '') {
            ...
            $idekspedisi = $this->db->query("SELECT f_idekspedisi_create() AS idekspedisi")->row()->idekspedisi;
            ...
            $simpan = $this->Ekspedisi_model->insert($data);
            if ($simpan) {
            ...
            $simpan = $this->Ekspedisi_model->update($data, $idekspedisi);
            if ($simpan) {
    ...
    public function getEditData()
    {
        ...
        $result                 = $this->Ekspedisi_model->getById($primaryKey)->row();

        $data = array(
            'idekspedisi' => $result->idekspedisi, 
            'namaekspedisi' => $result->namaekspedisi, 
    ...
    function getData()
    {
        $data       = $this->Ekspedisi_model->get_datatables();
        $dataArr    = array();
        $no         = $_POST['start'];
        ...
        foreach ($data as $row) {
            ...
            $arr[] = $row->idekspedisi.'<br>'.$statusaktif;

            $arr[] = $row->namaekspedisi;
            $arr[] = $row->keterangan;
            $arr[] = $row->link;
            ...
            $arr[]  = '
                        <a href="'.site_url( $this->controller.'/edit/'.$this->encrypt->encode($row->idekspedisi) ).'" class="btn btn-xs btn-warning btn-circle" title="Edit Data">
                            <i class="fa fa-edit"></i>
                        </a>  
                     
                        <a href="'.site_url( $this->controller.'/hapus/'.$this->encrypt->encode($row->idekspedisi) ).'" class="btn btn-xs btn-danger btn-circle" id="hapus" title="Hapus Data">
